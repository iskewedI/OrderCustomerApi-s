...
namespace OrderApi.Controllers.v1
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        /// <summary>
        ///     Action to retrieve a specific order
        /// </summary>
        /// <param name="Guid">Id of the order</param>
        /// <returns>Returns an order or none if not found.</returns>
        /// <response code="200">Returned if the order was retrieved</response>
        /// <response code="400">Returned if the order could not be retrieved</response>
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [HttpGet("{id}")]
        public async Task<ActionResult<Order>> Order(Guid id)
        {
            try
            {
